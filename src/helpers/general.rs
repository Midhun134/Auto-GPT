...
const CODE_TEMPLATE_PATH: &str =
    "/Users/midhu/rust-web-server-template/src/code_template.rs";

pub const WEB_SERVER_PROJECT_PATH: &str = "/Users/midhu/rust-web-server-template";

pub const EXEC_MAIN_PATH: &str =
    "/Users/midhu/rust-web-server-template/src/main.rs";

const API_SCHEMA_PATH: &str =
    "/Users/midhu/auto_gpt/schemas/api_schema.json";
...
pub async fn ai_task_request_decoded<T: DeserializeOwned>(
    ...
    // Add this line to debug the AI response
    //println!("AI Response: (Debug): {:?}", llm_response);
    // let decoded_response: T = match serde_json::from_str(llm_response.as_str()) {
    //     Ok(response) => response,
    //     Err(e) => panic!("Failed to decode AI response: {}. Response: {}", e, llm_response),
    // };
    //stripping the "'"'  wrapping because this might cause serde_json::from_str function to fail.(it actually was the problem!!)
    let sanitized_response = llm_response
        .trim()
        .trim_start_matches("```json")
        .trim_end_matches("```");
    let decoded_response: T = serde_json::from_str(sanitized_response)
